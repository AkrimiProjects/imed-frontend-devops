...
@Component({
  ...
  <section class="mx-auto max-w-7xl px-4 py-10 lg:px-8 space-y-8">
    ...
    <div *ngIf="activeTab() === 'internships'" class="rounded-3xl border border-border bg-white p-6 space-y-4">
      ...
      <button (click)="startQuiz()" [disabled]="uploadingCv() || submittingApplication() || !!validateApplicationForm()" class="inline-flex items-center rounded-2xl bg-teal-600 px-6 py-3 text-sm font-bold text-white hover:bg-teal-700 disabled:opacity-50">
        {{ submittingApplication() ? 'Envoi en cours...' : 'Passer le test QCM pour s’inscrire' }}
      </button>
    </div>

    <!-- Modal Quiz QCM -->
    <div *ngIf="showQuiz()" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div class="bg-white rounded-3xl border border-border p-8 w-full max-w-xl shadow-2xl space-y-6">
        <div class="flex justify-between items-center">
          <h2 class="text-xl font-extrabold text-teal-800">Test d'aptitude au stage</h2>
          <span class="text-xs font-bold text-muted-foreground uppercase">Question {{ currentQuestionIndex() + 1 }} / {{ quizQuestions.length }}</span>
        </div>
        
        <div class="space-y-4">
          <p class="text-lg font-semibold text-slate-800">{{ quizQuestions[currentQuestionIndex()].question }}</p>
          <div class="grid grid-cols-1 gap-3">
            <button *ngFor="let option of quizQuestions[currentQuestionIndex()].options; let i = index" 
                    (click)="selectAnswer(i)"
                    [ngClass]="userAnswers()[currentQuestionIndex()] === i ? 'border-teal-500 bg-teal-50 text-teal-700' : 'border-border bg-slate-50 hover:bg-slate-100'"
                    class="p-4 rounded-2xl border-2 text-left font-medium transition-all">
              {{ option }}
            </button>
          </div>
        </div>
        ...
        <div class="flex flex-col gap-3">
          <button (click)="nextQuestion()" 
                  ...
                  class="w-full bg-teal-600 hover:bg-teal-700 disabled:opacity-50 text-white rounded-2xl py-4 font-bold transition-all shadow-lg shadow-teal-600/20">
            {{ currentQuestionIndex() === quizQuestions.length - 1 ? 'Terminer et soumettre' : 'Question suivante' }}
          </button>
          <button (click)="showQuiz.set(false)" class="text-xs font-bold text-muted-foreground hover:text-slate-800 uppercase tracking-widest transition-colors">Abandonner</button>
        </div>
      </div>
    </div>
...
export class InternshipsComponent implements OnInit, OnDestroy {
  ...
  readonly quizPassed = signal<boolean | null>(null);

  readonly quizQuestions = [
    {
      question: "Quelle est l'importance de la ponctualité en entreprise ?",
      options: ["Secondaire", "Fondamentale pour le professionnalisme", "Facultative pour les stagiaires"],
      correct: 1
    },
    {
      question: "Si vous rencontrez une difficulté sur une tâche, que faites-vous ?",
      options: ["J'attends qu'on me demande", "Je cache le problème", "Je demande de l'aide après avoir essayé de résoudre"],
      correct: 2
    },
    {
        question: "Le travail d'équipe est-il essentiel ?",
        options: ["Oui, pour atteindre les objectifs communs", "Non, je préfère travailler seul", "Seulement si le chef est là"],
        correct: 0
    }
  ];
  ...
  finishQuiz(): void {
    const score = this.userAnswers().reduce((acc, ans, i) => {
      return acc + (ans === this.quizQuestions[i].correct ? 1 : 0);
    }, 0);

    const passed = score >= 2; // Pass if at least 2 correct answers
    this.quizPassed.set(passed);
    this.submitApplicationWithStatus(passed ? 'ACCEPTED' : 'REJECTED');
  }

  submitApplicationWithStatus(finalStatus: 'ACCEPTED' | 'REJECTED'): void {
    ...
    this.internshipApi.createApplication(payload).subscribe({
      next: () => {
        if (finalStatus === 'ACCEPTED') {
            this.successMessage.set('Félicitations ! Vous avez réussi le test et votre candidature est acceptée automatiquement.');
        } else {
            this.errorMessage.set('Malheureusement, vous n\'avez pas atteint le score requis. Votre candidature est refusée.');
        }
